...
def run_tavily_search(query: str) -> dict[str, Any]:
    ...
    try:
        ...
        raw = client.search(query=query)
        results = raw.get("results", [])
        if not results:
            return {"summary": "Keine Ergebnisse gefunden."}
        # Top-Ergebnis extrahieren und zusammenfassen
        top = results[0]
        summary = f"{top.get('title', '')}: {top.get('content', '')[:400]}"
        return {
            "summary": summary,
            "source": top.get("url", ""),
            "query": query
