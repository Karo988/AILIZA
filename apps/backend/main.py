...
def _ask_llm_directly(
    ...
    system_prompt = (
        ...
        "\n\n"
        "WICHTIG — DSGVO-Platzhalter: Texte können Platzhalter wie [Name], [Referenz-Nr.], [Wert], "
        "[E-Mail], [Firma], [Adresse], [Ort], [Datum], [IBAN], [Telefon] enthalten. "
        "Das sind Datenschutz-Platzhalter die AUTOMATISCH durch echte Werte ersetzt werden — "
        "NICHT nachfragen was sie bedeuten, NICHT erklären dass Infos fehlen. "
        "Schreibe den fertigen Text mit diesen Platzhaltern genau so wie sie sind. "
        "\n\n"
        "Bei Schreibaufgaben (E-Mail, Brief, Angebot, Entwurf) lieferst du IMMER sofort den fertigen Text — "
        "kein Nachfragen, keine Liste was noch fehlt, keine Erklärungen. Einfach schreiben. "
        "Wenn jemand eine Bestellbestätigung, PayPal-Zahlung oder Transaktionsdaten einfügt und eine "
        "E-Mail oder Antwort möchte, schreibst du IMMER eine fertige E-Mail AN DEN KUNDEN. "
        "Fehlende Details füllst du sinnvoll aus dem Kontext auf. "
        "Antworte immer auf Deutsch, präzise."
    )
    safe_history = []
    if history:
