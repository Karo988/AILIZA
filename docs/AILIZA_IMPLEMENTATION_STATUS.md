...
> **Nachprüfung 25.08.2026 gegen `c19cc1d`.**
> Vier Aussagen waren überholt und sind im Text als solche gekennzeichnet
> (durchgestrichen plus Nachtrag, nicht gelöscht): `.gitattributes`,
> `memory_items.tenant_id`, die Preview-Bindung des Streaming-Pfads sowie die
> PC-/Render-Standangaben. Zwei geprüfte Aussagen sind unverändert gültig:
> Obsidian hat weiterhin keinen Ist-Zustand im Repository, und die Tabellen
> `knowledge_approvals`/`claim_evidence` existieren nicht.
> Nicht nachgeprüfte Zeilen behalten ihren ursprünglichen Stand und sind
> **nicht** durch diese Nachprüfung bestätigt.
...
| `/agent/run/stream`, `/agent/approvals/{id}/continue` | ✅ **teilweise behoben** | `AgentRuntime.stream()` rief bisher `plan_tool_calls()` mit ROHEM Text auf, ganz ohne `classify()`/`redact()` — realer, von P0 unabhängiger Fund. Commit `0e9ed5e`: `stream()` nutzt jetzt denselben Precheck wie `run()`. ~~**Weiterhin nicht** durch den Preview-Beleg-Gate abgedeckt~~ — **überholt durch „Streaming-Governance (Paket D, Teil 2)" weiter unten in derselben Tabelle.** Verifiziert an `c19cc1d`: `/agent/run/stream` (nur POST, `main.py:3373`) verlangt einen gültigen Prüfbeleg über denselben `send_preview_store`; ohne `preview_id` endet der Aufruf mit `preview_invalid` (`main.py:1894-1928`). Ursprünglicher Stand 08.2026: nur classify/redact, keine Vorschau-Bindung — `continue_after_approval`/`stream_after_approval` führen nur bereits genehmigte Tool-Calls aus, kein neuer Nutzertext | Beleg-Gate auch für Streaming-Endpunkte, falls gewollt |
...
| **Streaming-Governance (Paket D, Teil 2)** | ✅ **Gate + Ausgangsprüfung wirksam** | `/agent/run/stream` verlangt jetzt serverseitig einen gültigen Prüfbeleg (derselbe `send_preview_store`, kein zweiter Belegweg). Die **GET-Variante ist entfernt** — sie transportierte den Aufgabentext als `?task=...` und damit in Zugriffsprotokolle, Browser-Verlauf und Referrer; das ist durch kein Backend-Gate reparierbar. Jedes Streaming-Ereignis läuft vor dem Senden durch `_gepruefter_ereignisstrom`; `tenant_id` ist an `sse_response()` ein Pflichtargument, damit eine künftige neue Route die Prüfung nicht vergessen kann. Die drei Fortsetzungs-Routen nach Freigabe sind ebenfalls angeschlossen (Freigabe einer Aktion ist keine Freigabe späterer Inhalte). Zusätzlich prüft `/agent/run` jetzt auch `steps` und `results` — Tool-Ergebnisse und abgerufene Webinhalte gingen bisher ungeprüft an die Anzeige. Belege: `tests/test_streaming_governance.py` (32 Tests), Gegenprobe gegen den alten Zustand = 8 rote Tests. |
| **Paket D, Teil 2 — Review-Nachbesserung** | ✅ fünf Funde plus zwei Nachbesserungen aus einem zweiten Review behoben | Unabhängiges Review deckte fünf reale Lücken auf, die trotz Teil-2-Gates noch offen waren: (1) die Such-Zusammenfassung sendete `search_text` ohne Governance-Kontext — Anbieter-Gate blieb blind für Daten, die nur im Suchtreffer standen; (2) der Gesprächsverlauf ging ungeprüft mit an den Anbieter, auch wenn die aktuelle Aufgabe harmlos war; (3) `_pruefe_nutzlast` prüfte nur Dictionary-**Werte**, nicht die Schlüssel selbst (`{"sk-...": "gefunden"}` kam durch); (4) `agent_runs`/`approval_requests` speicherten den Rohtext, bevor die Governance lief — Client-Ausgabe war geschützt, die Datenbank nicht; (5) die drei Freigabe-Fortsetzungsrouten prüften nur den Status der Freigabe-Nummer, nicht Mandant oder Eigentümerin — ein erratener Beleg fremder Nutzerin/fremden Mandanten war fortsetzbar. Fixes: gemeinsames Modul `governance/payload_check.py` (vermeidet zwei Fassungen derselben Prüfung zwischen `main.py` und `agent_runtime.py`), Kontextableitung schließt Suchtext und Verlauf ein, Schlüsselprüfung entfernt das Paar statt es umzubenennen, `sichere_fassung_fuer_speicherung()` vor jedem Schreiben in `agent_runs`, `_freigabe_zugriff_pruefen()` mit einheitlicher 404-Antwort (kein Unterschied zwischen "existiert nicht" und "gehört jemandem anderen"). Belege: `tests/test_governance_review_findings.py` (20 Tests), Gegenprobe = 14 rote Tests. `/documents/agent-run` ruft `run_agent()` direkt auf und erbt damit automatisch alle Fixes — keine separate Anbindung nötig. Ein zweites, unabhängiges Review der fünf Fixes fand zwei weitere kleine Lücken: (a) `_freigabe_zugriff_pruefen()` löste den Audit-Write nur bei Mandant-/Owner-Mismatch aus, nicht beim Fall "existiert nicht" — dadurch war per Antwortzeit unterscheidbar, ob eine Freigabe-Nummer überhaupt vergeben ist; jetzt schreibt jeder Ablehnungsfall symmetrisch. (b) `_schluessel_ist_unbedenklich()` behandelte Tupel-Schlüssel pauschal als unbedenklich, obwohl ein Tupel selbst eine Zeichenkette mit Secret enthalten kann; wird jetzt rekursiv geprüft. Ein zweiter Blick auf denselben Fix zeigte, dass `frozenset`-Schlüssel dieselbe Lücke wie Tupel hatten (nur `isinstance(..., tuple)` geprüft) — sofort mitgeschlossen. Belege: 3 weitere Tests in `tests/test_governance_review_findings.py` (23 insgesamt), Gegenprobe = alle drei rot gegen den alten Zustand. |
| **Vollständige Egress-Inventur** | ⚠️ teilweise veraltet | 9 Module mit Netzwerkausgang gelistet. **Behoben seit Paket D Teil 2:** `/agent/run/stream` (GET entfernt, POST verlangt Prüfbeleg), `/agent/approvals/{id}/continue(/stream)` (Ausgangsprüfung + Mandant-/Owner-Bindung), `/tools/search`, `/tools/fetch`. **Weiterhin offen:** Provider-Failover und der Prüfer-Zweitcall (`gated_client.py:310`) senden denselben Text an **weitere** Anbieter, der Beleg wird aber nur einmal am Eingang verbraucht. `/api/debug/groq-diagnosis` ist in **jeder** Umgebung registriert (fester Testprompt, kein Nutzinhalt) — anders als `/api/debug/provider-test`, das nur außerhalb Produktion registriert wird | Provider-Failover/Zweitcall an den zentralen Vertrag anbinden (Rest von Paket C) |
...
| Rohe Tool-Parameter in `approval_requests` | ✅ **behoben (Foundation-Phase 1, PR #89)** | Fund: die produktiv geladene Datei ist `apps/backend/gateway/runtime_gateway.py`, nicht `apps/backend/gateway.py` -- die Top-Level-Datei ist durch eine Namenskollision mit dem gleichnamigen Paket seit Langem tot/überschattet (bereits in `docs/ABSCHLUSSBERICHT.md:40-41` als bekannt dokumentiert, unverändert belassen). Neue Funktion `prepare_for_approval_storage()` in `governance/payload_check.py` (interner technischer Ausführungsspeicher, kein Egress-Entscheidungspunkt): operative Geschäftsdaten und gewöhnliche PII bleiben unverändert (kein Platzhaltersystem für Tool-Parameter -- eine Änderung würde später eine andere Aktion ausführen). Ein erkanntes Geheimnis führt zur sofortigen Ablehnung der Freigabeanfrage (422) -- im Repository existiert kein Secret-/Credential-Store (`routers/vault.py` ist ein Audit-Hashkettenspeicher, kein Secret-Vault), ein Platzhalter wäre eine stille Fehlausführung mit vermeintlichem Erfolg. Unbekannte/nicht bewertbare Typen (z. B. `bytes`, zu tiefe Verschachtelung) ergeben `clarification_required` (nicht `check_failed`/Secret-artig) -- kein Sicherheitsfund, sondern fehlende Information für eine sichere Entscheidung. **Sackgassen-Korrektur:** dieser Zustand brach zunächst den kompletten Agentenlauf ab (`AgentRuntime.run()`/`stream()` behandelten jede `HTTPException` aus dem Tool-Executor als `failed`/`blocked`). Jetzt erkennt `_is_clarification_required_error()` in `agent_runtime.py` GENAU dieses eine, über ein strukturiertes `HTTPException.detail`-Dict (`{"reason": "clarification_required", ...}`) signalisierte Ereignis -- kein pauschales Abfangen aller 422er, kein Textabgleich -- und lässt den Lauf mit Status `clarification_required` sowie einer konkreten Rückfrage enden, statt ihn abzubrechen. Bereits erfolgreich verarbeitete Schritte bleiben erhalten. Policy-Blöcke (403) und andere, nicht strukturiert signalisierte 422-Fehler verhalten sich unverändert wie zuvor (Lauf bricht ab). **Secret-UX-Finalisierung (Betreiber-Freigabe "OK Secret-UX-Finalisierung"):** ein erkanntes Geheimnis bricht ebenfalls nicht mehr den gesamten Agentenlauf ab. `runtime_gateway.py` signalisiert `ablehnungsgrund == "secret_detected"` jetzt über ein eigenes strukturiertes `HTTPException.detail` (`{"reason": "credential_input_blocked", ...}`) statt eines reinen Strings; `_is_credential_input_blocked_error()` in `agent_runtime.py` erkennt GENAU dieses Signal (kein pauschales 422-Fangen) und lässt `run()`/`stream()` mit Status `credential_input_blocked` sowie `next_action: "remove_credentials_and_retry"` enden, statt zu werfen. Anders als `clarification_required` ist dieser Zustand NICHT technisch fortsetzbar (kein sicherer Credential-Kanal vorhanden) -- der Lauf endet kontrolliert, bereits erfolgreich verarbeitete Schritte bleiben erhalten. Das Secret selbst steht nirgends in `message`/`steps`/`results`/Audit -- unverändert: `payload_check.py` speichert bei einem Secret-Fund weiterhin `parameter={}`. `payload_check.py` selbst wurde für diese Korrektur NICHT geändert (nur die HTTP-Signalisierung in `runtime_gateway.py` und die Auswertung in `agent_runtime.py`). Frontend (`AgentChat.jsx`/`App.css`): eigener, von generischen Fehlern unterscheidbarer Warnzustand (gelb/orange, "⚠️ Sicherheitshinweis"), keine Erfolgsoptik. Belege: `tests/test_agent_runtime_clarification.py` (18 Tests), `tests/test_gateway_approval_storage.py` (Secret-Detail-Test ergänzt). **Special Category (Art. 9/10 DSGVO) wird intern NICHT blockiert** -- interne Verarbeitung ist kein Egress: `prepare_for_approval_storage()` entscheidet nur über einen internen technischen Speicher, nicht über eine externe Weitergabe. Besondere Kategorien werden klassifiziert und maschinenlesbar markiert (`special_category_erkannt`/`special_category_detected` im Audit), aber normal gespeichert. Ob eine externe Weitergabe zulässig ist, entscheidet separat die Egress-Governance (Paket D) -- ein `responsibility_handoff` bei nicht automatisch freigegebenem Egress ist ein eigenes, noch nicht begonnenes Arbeitspaket. Kanonische AILIZA-Redaktionsmarker (z. B. `[GESCHWAERZT: Gesundheit - Art. 9 DSGVO]`) werden für die erneute Klassifikation über `RedactionEngineV2.canonical_violet_markers()` neutralisiert, damit der Marker selbst (der wörtlich den Kategorienamen enthält) nicht fälschlich erkannt wird -- Secret-Prüfung bleibt immer auf dem Original, gespeicherte/ausgeführte Parameter bleiben unverändert, ein erfundener Marker bleibt klassifizierbar. Derselbe Textfragment-Scanner gilt für Dict-Schlüssel und -Werte. Belege: `tests/test_gateway_approval_storage.py` (36 Tests). | Rest-Risiko dokumentiert: Tenant-/Owner-Bindung bei der Approval-**Erstellung** in diesem Gateway-Pfad existiert weiterhin nicht (`request_approval_if_needed()` übergibt weder `tenant_id` noch `owner_user_id`); Responsibility-Handoff bei nicht automatisch freigegebenem Egress ist ein eigenes, separat zu beauftragendes Arbeitspaket. |
| PC synchronisieren | ⚠️ | **Stand 14.08.2026, seither überholt** — beide genannten Commits (`ea2cbd6`, `1e6bda7`) liegen weit hinter dem heutigen `main` (`c19cc1d`, 25.08.2026). Die Angabe ist als PC-Synchronisationsstand wertlos; PC-Synchronisierung ist aus dieser Cloud-Sitzung nicht prüfbar (kein Zugriff auf C:\AILIZA) | Auf dem PC: lokale Änderungen sichern, dann per Fast-forward auf den **aktuellen** `origin/main` synchronisieren (nicht auf `1e6bda7`) |
| Render-Deployment | ⚠️ | ✅ Staging erreichbar: `https://ailiza-stagin.onrender.com/`, Oberfläche meldet „System aktiv". ⚠️ deployter Commit unbekannt, ⚠️ Render-Branch unbekannt, ⚠️ Datenbanktyp unbekannt, ⚠️ Alembic-Revision der Staging-Datenbank unbekannt. Render läuft laut Stand 14.08.2026 auf `main`/`058fea1` — **seither überholt**, `main` steht heute auf `c19cc1d`. Der deployte Commit ist damit unbekannt **und** mit Sicherheit veraltet | Render-Dashboard prüfen: deployter Commit, Branch, DB-Typ, Alembic-Revision |
...
| `memory_items.tenant_id` NOT NULL | ✅ **erledigt** | **Stand 14.08.2026 („weiterhin nullable") seither überholt.** Verifiziert an `c19cc1d`: `db_schema.py:303-310` — NOT NULL für ALLE Scopes, Migration `c8ff9bb332ba` erzwingt das per fail-closed Guard (bricht bei bestehenden NULL-Zeilen kontrolliert ab, kein automatischer Default-Tenant, kein Backfill). Ursprünglicher Eintrag: nullable wegen alter `user_memory`-Datensätze (`legacy_user_memory_null_tenant`). Empfehlung dieser Session: Ja, langfristig NOT NULL — aber Migration muss bei nicht eindeutig zuordenbaren Altdaten abbrechen und darf niemals automatisch `default` setzen | Backfill-Strategie ausarbeiten (owner_user_id + source_id müssen übereinstimmen), dann eigene Migration |
| CRLF / Gate 10 | ⚠️ | Ursache (CRLF vs. echte Änderung) bisher nicht am identischen Commit auf Windows bewiesen; ~~`.gitattributes` existiert nicht im Repo~~ — **Stand 14.08.2026, seither überholt:** `.gitattributes` existiert seit PR #97 (21.08.2026) und normalisiert genau die acht Gate-10-Dateien auf LF (verifiziert an `c19cc1d`, 1028 Bytes) | Nur bei tatsächlich rotem Gate 10 erneut untersuchen |
