...
| **P0 — Chat-Schutzgate: Backend** | ✅ wirksam für `/agent/run` | Commits `937f4ce` (Modul, 26 Tests) + `98149be` (Verdrahtung, 10 Integrationstests). `/api/policy-redact` stellt `preview_id` aus, `/agent/run` löst sie ein; bei Manipulation, fremder Anmeldung/Mandant/Zweck, Ablauf oder Wiederverwendung bricht der Lauf ab, **bevor** ein Provider kontaktiert wird (am Provider-Mock belegt). Volle Suite 1484 passed/0 failed. Reviews: auth-security-reviewer (kein Loch, 1 Fund behoben), pr-diff-reviewer GO | Frontend-Zustandsautomat, damit die Vorschau auch benutzt wird |
| **P0 — Chat-Schutzgate: Frontend** | 🔄 offen | Die Lupe ist im Frontend weiterhin optional/nicht-blockierend; ohne `preview_id` läuft der bisherige Serverweg. Der Schutz greift also erst, wenn das Frontend die Vorschau tatsächlich nutzt | Zustandsautomat (geprüft/geändert/ungültig), Senden an geprüften Stand koppeln |
