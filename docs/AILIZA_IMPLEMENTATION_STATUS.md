...
| **P0 — Chat-Schutzgate: Backend + Frontend** | ✅ **verpflichtend, für `/agent/run` wirksam** | Commits `937f4ce`, `98149be`, `0457e71` (verpflichtend + Gate-Position korrigiert + Rohtext-Bindung), `6d63f63` (Frontend). Ohne gültigen Beleg: `status="preview_invalid"`, kein Provider-Call (am Mock belegt). Ausnahme bleibt bewusst: `consent_approval_id`-Pfad (Fall 3) hat eigene `task_sha256`-Bindung. `/documents/agent-run` stellt sich selbst einen Beleg für den serverseitig komponierten Text aus. Frontend holt den Beleg automatisch vor jedem Senden, kein manueller Schritt mehr nötig. Volle Suite 1486 passed/0 failed. Reviews: auth-security-reviewer (Gate-Position lückenlos verifiziert, Rohtext-Bindung korrekt, keine Race Condition), pr-diff-reviewer GO | Output-Governance (siehe unten); `/agent/run/stream` + `/agent/approvals/{id}/continue` bleiben außerhalb dieses Gates |
| **P0 — Output-Governance** | 🔄 offen | Antwort wird vor Reinsertion/Anzeige nicht erneut klassifiziert | siehe Einzelpunkte unten |
| `/agent/run/stream`, `/agent/approvals/{id}/continue` | ⚠️ offen, **neuer Fund** | Laufen über `AgentRuntime.stream()`, komplett außerhalb von `_run_agent_core` — das P0-Gate erreicht diese Pfade nicht. Ungeprüft, ob dort überhaupt eine Governance-Pipeline existiert | Eigene Prüfung von `AgentRuntime`, bevor P0 als vollständig gilt |
