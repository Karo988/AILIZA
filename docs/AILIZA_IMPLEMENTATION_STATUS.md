...
| Rohe Tool-Parameter in `approval_requests` | ✅ **behoben (Foundation-Phase 1)** | Fund: die produktiv geladene Datei ist `apps/backend/gateway/runtime_gateway.py`, nicht `apps/backend/gateway.py` -- die Top-Level-Datei ist durch eine Namenskollision mit dem gleichnamigen Paket seit Langem tot/überschattet (bereits in `docs/ABSCHLUSSBERICHT.md:40-41` als bekannt dokumentiert, unverändert belassen). Neue Funktion `prepare_for_approval_storage()` in `governance/payload_check.py`: klassifiziert Tool-Parameter vor der Persistenz, ohne pauschale Redaction. Operative Geschäftsdaten und gewöhnliche PII bleiben unverändert (kein Platzhaltersystem für Tool-Parameter -- eine Änderung würde später eine andere Aktion ausführen). Ein erkanntes Geheimnis führt zur sofortigen Ablehnung der Freigabeanfrage (422, verständliche Meldung) statt zur stillen Entfernung -- im Repository existiert kein Secret-/Credential-Store (`routers/vault.py` ist ein Audit-Hashkettenspeicher, kein Secret-Vault), ein Platzhalter wäre eine stille Fehlausführung mit vermeintlichem Erfolg gewesen. Ein Sicherheitsreview fand danach einen Randfall: `bytes`-Werte (kein JSON-Typ, praktisch selten erreichbar) wurden vom Scanner stillschweigend als unauffällig behandelt statt fail-closed -- sofort behoben. **Finalisierung (Betreiber-Freigabe):** besondere Kategorien (Art. 9/10 DSGVO) werden in diesem generischen Approval-Pfad jetzt ebenfalls blockiert (`special_category_no_binding_context`), nicht mehr nur markiert -- solange der Pfad keinen belastbaren Tenant-/Owner-Kontext besitzt, ist eine rohe Persistenz unkontrolliert. Ausdrücklich keine generelle Aussage: besondere Kategorien sind in autorisierten, tenant-/ownergebundenen internen Geschäftsspeichern nicht grundsätzlich verboten -- die Regel gilt nur für diesen konkreten, ownerlosen technischen Speicher und ist neu zu bewerten, sobald er einen Kontext erhält. Reihenfolge: ein Fund mit Geheimnis UND besonderer Kategorie wird als `secret_detected` gemeldet (konkretere Diagnose gewinnt). **Nachbesserung (Fall-2-Regression):** die Blockierung fing zunächst auch bereits sicher redigierten Text ab -- der AILIZA-Redaktionsmarker `[GESCHWAERZT: Gesundheit - Art. 9 DSGVO]` enthält wörtlich den Kategorienamen und wurde vom Classifier erneut als SPECIAL_CATEGORY erkannt. `RedactionEngineV2.canonical_violet_markers()` (neuer öffentlicher Helper) liefert jetzt die exakte Markermenge; `payload_check.py` neutralisiert NUR diese exakten Strings für die erneute Klassifikation (nicht für Secret-Prüfung, nicht für die gespeicherte/ausgeführte Nutzlast) -- keine Wildcard-Ausnahme, ein erfundener Marker bleibt klassifizierbar. Gleichzeitig wird derselbe Textfragment-Scanner jetzt auch auf Dict-Schlüssel angewendet (vorher nur Werte). Belege: `tests/test_gateway_approval_storage.py` (29 Tests), zwei Gegenproben in beide Richtungen: ohne Neutralisierung wird `test_fall2_pii_logged_in_passes_gates` rot, ohne Special-Category-Blockierung werden 7 der neuen Special-Category-Tests rot. | Rest-Risiko dokumentiert: Tenant-/Owner-Bindung bei der Approval-**Erstellung** in diesem Gateway-Pfad existierte bereits vorher nicht (`request_approval_if_needed()` übergibt weder `tenant_id` noch `owner_user_id`) und wurde durch diese Änderung weder eingeführt noch behoben -- Regressionstest sichert nur den unveränderten Ist-Zustand ab. |
