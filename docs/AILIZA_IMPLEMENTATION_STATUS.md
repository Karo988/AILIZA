...
| **P0 — Chat-Schutzgate: Backend + Frontend** | ✅ **verpflichtend, für `/agent/run` wirksam, Consent+Preview beide Pflicht** | Commits `937f4ce`, `98149be`, `0457e71`, `6d63f63`, `51de03f`. Ohne gültigen Beleg: `status="preview_invalid"`, kein Provider-Call (am Mock belegt). `consent_approval_id`-Pfad (Fall 3) braucht jetzt **zusätzlich** einen Beleg — vorher übersprang Consent die Beleg-Prüfung komplett (Sicherheitsreview-Fund, behoben: preview_id wird vor Consent-Verbrauch geprüft, damit keine Einwilligung "verbrannt" wird, ohne dass gesendet wurde). `/documents/agent-run` stellt sich selbst einen Beleg für den serverseitig komponierten Text aus. Frontend holt den Beleg automatisch vor jedem Senden. Volle Suite 1490 passed/0 failed. Reviews: auth-security-reviewer (2 Funde behoben, danach commit-reif), pr-diff-reviewer GO | Output-Governance (siehe unten) |
